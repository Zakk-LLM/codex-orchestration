...
# This fixture keeps only Codex event shapes: no recorded paths, commands, or output.
fresh || exit 2
...
def completed(item_type, item_id, **fields):
    item = {"id": item_id, "type": item_type}
    item.update(fields)
    return json.dumps({"type": "item.completed", "item": item},
                      separators=(",", ":")) + "\n"

def event(call, *, ok=True, args=None):
    command = json.dumps(args, separators=(",", ":")) if args is not None else f"tool-{call}"
    return completed("command_execution", call, command=command,
                     exit_code=0 if ok else 1, status="completed")

def fixture():
    return "".join([
        json.dumps({"type": "item.started", "item": {
            "id": "started", "type": "command_execution", "command": "pending",
            "exit_code": None, "status": "in_progress"}}, separators=(",", ":")) + "\n",
        completed("agent_message", "message", text="omitted"),
        completed("command_execution", "command-ok", command="inspect",
                  exit_code=0, status="completed"),
        completed("command_execution", "command-error", command="retry",
                  exit_code=2, status="completed"),
        completed("command_execution", "command-declined", command="denied",
                  exit_code=None, status="declined"),
        completed("file_change", "file-ok",
                  changes=[{"path": "one.txt"}, {"path": "two.txt"}], status="completed"),
        completed("mcp_tool_call", "mcp-error", server="docs", tool="read",
                  arguments={"topic": "api"}, status="failed"),
        completed("web_search", "web", query="event protocol"),
        "{broken json\n",
    ])
...
def dispatch_fake(name, lines, *, delay=0, exit_code=0, max_tools=None, timeout=4):
    bindir = fake_codex(lines, delay, exit_code)
    run_dir = tmp / name
    env = os.environ | {
        "PATH": f"{bindir}:{os.environ['PATH']}",
        "AGENT_START_STAGGER": "0",
        "CODEX_REGISTRY_DIR": str(tmp / f"{name}-registry"),
        "AGENT_SLOTS_DIR": str(tmp / f"{name}-slots"),
        "AGENT_ORCHESTRATION_ENV": str(tmp / "no-agent-env"),
    }
    command = [root / "scripts/codex_agent.sh", "--run-dir", run_dir, "--label", "w",
               "--prompt", "x", "--admission", "off", "--timeout", str(timeout)]
    if max_tools is not None:
        command += ["--max-tools", str(max_tools)]
    result = subprocess.run(command, env=env, capture_output=True, text=True)
    meta = run_dir / "agents/w/meta.json"
    return result, json.loads(meta.read_text()) if meta.exists() else {}

def parser_boundaries():
    path = tmp / "events.jsonl"
    body = fixture()
    half = completed("file_change", "half",
                     changes=[{"path": "later.txt"}], status="failed")
    path.write_text(body + half.rstrip())
    rows, offset = scan_tools(path, 0)
    assert [row["name"] for row in rows] == [
        "command_execution", "command_execution", "command_execution",
        "file_change", "docs/read", "web_search"]
    assert [row["ok"] for row in rows] == [True, False, False, True, False, True]
    assert rows[0]["args_head"] == "inspect"
    assert rows[3]["args_head"] == "one.txt,two.txt"
    assert rows[4]["args_head"] == '{"topic":"api"}'
    assert rows[5]["args_head"] == "event protocol"
    assert all(len(row["args_head"]) <= 80 for row in rows)
    assert offset == len(body.encode())
    more, same_offset = scan_tools(path, offset)
    assert more == [] and same_offset == offset
    with path.open("a") as stream:
        stream.write("\n")
    more, final_offset = scan_tools(path, offset)
    assert len(more) == 1 and more[0]["ok"] is False
    whole, whole_offset = scan_tools(path, 0)
    assert whole == rows + more and whole_offset == final_offset == path.stat().st_size
    reset, reset_offset = scan_tools(path, final_offset + 1)
    assert reset == whole and reset_offset == final_offset

def wrapper_counts():
    lines = [event("ok").strip(), event("failed", ok=False).strip(),
             completed("command_execution", "declined", command="denied",
                       exit_code=None, status="declined").strip()]
    result, meta = dispatch_fake("agent-run", lines)
...
def watch_incremental_identity():
    ...
    events.write_text(event("a", args={"value": "x" * 500}))
    started_at = int(time.time())
    (agent / "started.json").write_text(json.dumps(
        {"started_at": started_at, "deadline": started_at + 1000, "timeout_s": 1000}))
    command = [root / "scripts/codex_watch.sh", run_dir, "--timeout", "0", "--interval", "1",
    ...
    events.write_text(event("b") + event("c"))
...
def make_watch(name, successes, failures_count=0, age=0, deadline=2000, label="w"):
    run_dir = tmp / name
    agent = run_dir / f"agents/{label}"
    agent.mkdir(parents=True)
    rows = [event(f"s{i}") for i in range(successes)]
    rows += [event(f"f{i}", ok=False) for i in range(failures_count)]
...
def poll_watch(run_dir, tools=100, minutes=45):
    return subprocess.run(
        [root / "scripts/codex_watch.sh", run_dir, "--timeout", "0", "--interval", "1",
         "--reflect-tools", str(tools), "--reflect-min", str(minutes)],
        capture_output=True, text=True)

def reflect_threshold_and_dedup():
    run_dir = make_watch("threshold-run", 99, failures_count=5)
    first = poll_watch(run_dir)
    assert first.returncode == 1 and "REFLECT" not in first.stdout
    with (run_dir / "agents/w/events.jsonl").open("a") as output:
        output.write(event("hundred"))
    ...
    with (run_dir / "agents/w/events.jsonl").open("a") as output:
        output.write("".join(event(f"later{i}") for i in range(100)))
...
def reflect_single_and_deadline():
    ...
    assert shlex.split(advertised) == [
        "codex_reflect.sh", str(elapsed), "w;echo bad", "--trigger", "elapsed"]
...
def max_tools_slow():
    lines = [event(str(index)).strip() for index in range(11)]
...
def reflect_run(name, value, *, extra_events=(), env_extra=None, dry=False, exit_code=0,
    ...
    command = [root / "scripts/codex_reflect.sh", run_dir, "w"]
    if trigger:
...
def valid_no_issue():
    value = "```json\n" + json.dumps({
        "verdict": "NO_ISSUE", "reason": "The route matches."}) + "\n```"
...
def valid_route():
    value = json.dumps({
        "verdict": "ROUTE_CORRECTION", "reason": "A stop was bypassed.",
        "next_step": "Stop and report.",
...
def failed_reflector():
    count = tmp / "reflect-failed-count"
    value = json.dumps({"verdict": "NO_ISSUE", "reason": "x"})
    run_dir, result, _ = reflect_run(
        "reflect-exit", value, exit_code=1, env_extra={"FAKE_COUNT": str(count)})
    assert result.returncode != 0 and count.read_text().splitlines() == ["1"]
    assert (run_dir / "agents/w/reflect-1.error").exists()
    assert (run_dir / "reflect/w-1/agents/reflector/meta.json").exists()

def dry_run_prompt():
    def prepare(_, worker):
        rows = [event(f"d{index}", ok=index != 6, args={"index": index})
                for index in range(45)]
        (worker / "events.jsonl").write_text("".join(rows))
        (worker / "reflect-7.json").write_text(json.dumps(
            {"verdict": "NO_ISSUE", "reason": "history"}))
    run_dir, _, state = reflect_run(
        "reflect-dry", json.dumps({"verdict": "NO_ISSUE", "reason": "x"}),
        dry=True, prepare=prepare)
    prompt = (run_dir / "agents/w/reflect-8.prompt.md").read_text()
    heads = [prompt.index(value) for value in (
        "## prompt.md", "## maintainer.md", "## NOTES.md", "## Tool summary")]
...
def admission_preserves_pending():
    value = json.dumps({"verdict": "NO_ISSUE", "reason": "x"})
    run_dir, result, state = reflect_run(
        "reflect-full", value, env_extra={"CODEX_MAX_AGENTS": "0"})
    assert result.returncode == 3
    assert json.loads((run_dir / ".watch-state").read_text()) == state
    assert not (run_dir / "agents/w/reflect-1.error").exists()

def over_budget_is_error():
    tools = [event(str(index)).strip() for index in range(11)]
    value = json.dumps({"verdict": "NO_ISSUE", "reason": "x"})
...
def lock_and_numbering():
    ...
    with lock.open("w") as held:
        ...
        _, result, state = reflect_run(
            "reflect-lock", json.dumps({"verdict": "NO_ISSUE", "reason": "x"}))
...
def completion_uses_current_count():
    ...
    def mutate():
        time.sleep(1)
        with (tmp / name / "agents/w/events.jsonl").open("a") as output:
            output.write("".join(event(f"new{index}") for index in range(100)))
    value = json.dumps({"verdict": "NO_ISSUE", "reason": "x"})
...
def timeout_does_not_redrive():
    ...
    start = time.time()
    run_dir, result, _ = reflect_run(
        "reflect-timeout", value, delay=20,
...
def status_fixture(name, running=False):
    ...
    agent.mkdir(parents=True)
    if running:
        (agent / "events.jsonl").write_text(event("a"))
        now = int(time.time())
        (agent / "started.json").write_text(json.dumps(
...
def status_running_state():
    result = subprocess.run(
        [root / "scripts/codex_status.sh", status_fixture("status-running", True), "--brief"],
        capture_output=True, text=True)
...
def status_finished_reports():
    ...
    for flag in ("--brief", "--full"):
        result = subprocess.run(
            [root / "scripts/codex_status.sh", run_dir, flag], capture_output=True, text=True)
...
def nested_runs_stay_isolated():
    ...
    (nested / "meta.json").write_text(json.dumps(
        {"exit_code": 0, "worktree_branch": "codex/child"}))
    wait = subprocess.run(
        [root / "scripts/codex_wait.sh", run_dir, "--timeout", "0"],
        capture_output=True, text=True)
    assert wait.stdout.strip() == "w OK"
    parent_meta = run_dir / "agents/w/meta.json"
    meta = json.loads(parent_meta.read_text())
    meta.update({"worktree_branch": "codex/parent", "base_sha": "", "cwd": str(tmp)})
    ...
    merge = subprocess.run(
        [root / "scripts/codex_merge.sh", "--run-dir", run_dir,
...
def new_run_prompts_for_maintainer_words():
    base = tmp / "new-runs"
    result = subprocess.run(
        [root / "scripts/codex_new_run.sh", "probe"],
        env=os.environ | {"CODEX_RUNS_DIR": str(base)}, capture_output=True, text=True)
    assert result.returncode == 0
    assert "maintainer.md" in (pathlib.Path(result.stdout.strip()) / "PLAN.md").read_text()

phase = sys.argv[2]
if phase == "events":
    run("event parser maps terminal Codex tools and complete lines", parser_boundaries)
    run("agent meta counts terminal tools from the parser", wrapper_counts)
    ...
    run("reflect rejects an absent quote", lambda: invalid_result(
        "badquote", {"verdict": "ROUTE_CORRECTION", "reason": "x", "next_step": "y",
                     "quotes": [{"source": "maintainer.md", "text": "ABSENT"}]}))
    run("reflect rejects an empty quote", lambda: invalid_result(
        "emptyquote", {"verdict": "ROUTE_CORRECTION", "reason": "x", "next_step": "y",
                       "quotes": [{"source": "maintainer.md", "text": "   "}]}))
    run("reflect rejects source misuse", lambda: invalid_result(
        "misuse", {"verdict": "ROUTE_CORRECTION", "reason": "x", "next_step": "y",
                   "quotes": [{"source": "maintainer.md", "text": "SPEC SENTENCE"}]}))
    run("reflect rejects a missing next_step", lambda: invalid_result(
        "next", {"verdict": "ROUTE_CORRECTION", "reason": "x",
                 "quotes": [{"source": "maintainer.md", "text": "STOP HERE"}]}))
...
fresh || exit 2
printf '\ncase x in\n' >> "$TMP/w/scripts/codex_reflect.sh"
expect 1 "codex_reflect.sh does not parse" sh "$TMP/w/scripts/check-shell-syntax.sh"
