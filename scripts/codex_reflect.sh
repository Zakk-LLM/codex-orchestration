...
# codex_note.sh in their own words. Nothing here kills or re-dispatches anything.
#
# Bounds are enforced, not requested: ten completed tools via the wrapper's --max-tools and
# the recount after exit, 390 seconds total (300 for the model, 30 seconds of termination grace,
# and 60 seconds of script headroom), one launch (AGENT_LOCK_RETRIES=1). The result is read from
# last.txt and validated here so exact quotes can be checked against their local sources.
set -uo pipefail

usage() {
  cat <<'EOF'
Usage: codex_reflect.sh <run-dir> <label> [options]
...
TOOLS_AT_CHECK=$(PYTHONPATH="$HERE" python3 -c \
  'from codex_events import scan_tools; import sys; print(sum(x["ok"] for x in scan_tools(sys.argv[1], 0)[0]))' \
  "$WORKER/events.jsonl")
...
ARGS=(--run-dir "$REFLECT_RUN" --label reflector --prompt-file "$PROMPT_OUT"
      --sandbox read-only --admission refuse --timeout 300 --max-tools 10 --tier "$TIER"
...
if [ "$REMAINING" -le 0 ]; then
  ...
  AGENT_START_STAGGER=0 AGENT_LOCK_RETRIES=1 \
    timeout --signal=KILL "$REMAINING" "$HERE/codex_agent.sh" "${ARGS[@]}"
  AGENT_CODE=$?
fi
