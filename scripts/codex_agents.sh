...
PY
  ...
  --count|--list|--prune|--slots)
    ...
    AGENT_ENGINE_FILTER="$ENGINE_FILTER" python3 - "$REG" "$ACTION" "${AGENT_MAX_AGENTS:-${CODEX_MAX_AGENTS:-5}}" <<'PY'
...
def scan_unregistered(known):
    ...
    for entry in pathlib.Path("/proc").iterdir():
        ...
        # Both engines count: `codex exec` (alias `e`, root options may precede it) and
        # `opencode run`. Anything else with that name is a TUI or an unrelated program.
