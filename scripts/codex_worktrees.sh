...
Usage: codex_worktrees.sh <run-dir> --list
       ...
       codex_worktrees.sh <run-dir> --drift [BASE]
       codex_worktrees.sh <run-dir> --rebase [BASE]
...
case "$ACTION" in
  ...
  --drift|--rebase)
    ...
    for wt in "$RUN"/worktrees/*/; do
      [ -d "$wt" ] || continue
      name=$(basename "$wt")
      behind=$(git -C "$REPO" rev-list --count "codex/$name..$BASE" 2>/dev/null || echo 0)
...
reg = pathlib.Path(os.environ.get("CODEX_REGISTRY_DIR",
      os.environ.get("XDG_RUNTIME_DIR", "/tmp") + "/codex-agents"))
...
for entry in pathlib.Path("/proc").iterdir():
    ...
    try:
        ...
        continue
    if "codex" in argv and wt in argv:
        print("yes"); raise SystemExit
print("no")
LIVE
)
