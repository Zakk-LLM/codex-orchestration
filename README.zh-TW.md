...
| `medium` | 預設值：範圍受限的功能、文件、單一模組的測試 | 900–1800 |
| `high` | 跨多檔案的修改、難以定位的缺陷、需保持行為的重構 | 1800–3600 |
| `xhigh` | 架構決策、並行與效能問題、需求含糊 | 3600–7200 |
| `max` | `xhigh` 在同一問題上失敗兩次後的最後手段 | 7200 以上 |
...
重試也要分類。連線中斷、API 錯誤、行程被結束屬於暫時性失敗，可直接續作；語義失敗不重送相同提示，改為附上審查證據的定向修正，或重新界定範圍後另派代理。
