...
> **已併入 dispatch。** 本 skill 併進 [dispatch](https://github.com/Zakk-LLM/dispatch)，以 `--engine codex` 使用；新工作到那邊。本倉庫原樣保留，舊的 run 目錄仍可讀（tag `v1-standalone`）。
...
有些工作不派發。git 機械操作（rebase、合併、解衝突、commit、分支與工作區清理）由協調者執行，而且每份任務規格都禁止工作代理執行這些命令，派發等於自相矛盾；`codex_worktrees.sh --rebase` 與 `codex_merge.sh` 一條命令就能完成。另外三類同樣不派發：修正本身比撰寫規格還快、改動小到審查就是全部成本、以及執行一條命令並讀取輸出。判準是獨立的上下文視窗能否抵過規格加審查的成本：同一個重新命名套用到 200 個檔案值得派發，套用到 3 個檔案不值得。派發瑣碎任務還有兩重代價，它佔用機器上限計入的名額，並把一個審查插到真正需要審查的代理前面。

## 順序與原子性

兩者出自同一條規則：工作只能建立在確實存在的狀態上，也就是依賴任務已完成的結果，或目標分支真實的 commit。
...
有依賴的任務要等依賴全部成功才派發，其中任何一個失敗就直接跳過。提早派發的代價是它針對尚不存在的結構或簽名工作，整輪成果作廢。未知的依賴與循環依賴在派發任何代理之前就被拒絕；某個依賴失敗只終止它自己的子樹，互相獨立的分支繼續執行。

整合沿用同一順序：分支依相依順序合併，每次合併都先驗證再處理下一個，任何失敗都把目標分支還原到本次執行開始時的 commit。
...
監控是事件驅動而非定時輪詢：這個呼叫會阻塞到狀態改變才返回，`--interval` 只決定每隔多久檢查幾個檔案，不消耗 token 也不消耗回合。

等待期間幾乎不讀取資料。存活判斷取事件記錄的 mtime，加上 `--peek` 時只讀檔案最後 4 KB 取出最後一條事件，3 MB 的記錄與 3 KB 的記錄成本相同，而且只在內容變化時輸出，也不會被誤判為狀態改變。事件記錄與結果留在磁碟上，直到確實需要才讀：結果只在審查時讀一次，`codex_status.sh --brief` 則只印表格，不含任何結果內容。

它同時盯著每個代理的時鐘。派工時已記錄截止時間，因此警告在保護機制動作之前送達：用掉八成時限時回報 `EXPIRING <n>s left`（`--warn` 可調整比例），接近停滯上限時回報 `QUIET <n>s without an event`。收到後要在工作還存在時處理——透過 `NOTES.md` 要求工作代理停止探索並回報現有成果，或先準備續作規格——因為保護機制結束行程會丟掉進行中的那一輪，那些工作等於付兩次錢。首次執行就出現 `EXPIRING` 也是範圍訊號：任務比選定的時限大，續作應該拆分而不是單純加時間。執行中的代理在 `codex_status.sh` 的 TIME 欄顯示剩餘時間而非已用時間。

離開碼 0 表示有代理改變狀態，並印出 label。1 表示時限內沒有變化，這段時間用於不依賴代理的工作。2 表示整批已完成。3 表示尚未派工。

離開碼 1 是指示，不是再呼叫一次的理由。時限依距離下一件有用的事還有多久設定，不依代理可能執行多久。宿主環境若能主動喚醒，優先使用喚醒機制，並保留有時限的等待作為後備，因為靜默失敗會讓整批無限期停住。
